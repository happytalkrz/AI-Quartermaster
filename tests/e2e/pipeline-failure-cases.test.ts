...
import { createCheckpoint, rollbackToCheckpoint, ensureCleanState } from "../../src/safety/rollback-manager.js";
...
function setupStateCapture(): void {
  ...
  mockInitializePipelineState.mockResolvedValue({
    ...
    worktreePath: "/tmp/project/.aq-worktrees/42-test",
  } as PipelineRuntime);
}
...
describe("E2E: Pipeline Failure Cases", () => {
  beforeEach(() => {
    ...
    // Reset rollback mocks
    mockCreateCheckpoint.mockResolvedValue("abc123def456");
    mockRollbackToCheckpoint.mockResolvedValue();
    mockEnsureCleanState.mockResolvedValue({ path: "/tmp/project/.aq-worktrees/42-test", branch: "aq/42-fix-bug" });
  });
  ...
  describe("Core Loop Failures", () => {
    it("should handle plan generation failure", async () => {
      ...
      // Plan generation failure triggers rollback through general error handler
      expect(mockRollbackToCheckpoint).toHaveBeenCalledWith(
        "abc123def456",
        { cwd: "/tmp/project/.aq-worktrees/42-test", gitPath: "git" }
      );
    });
    ...
    it("should handle phase execution failure", async () => {
      ...
      // Phase execution failure should trigger rollback (checkpoint exists, rollbackStrategy is "all")
      expect(mockRollbackToCheckpoint).toHaveBeenCalledWith(
        "abc123def456",
        { cwd: "/tmp/project/.aq-worktrees/42-test", gitPath: "git" }
      );
  ...
  describe("Review Failures", () => {
    it("should handle review rejection", async () => {
      ...
      // Review failure may trigger rollback depending on pipeline configuration
      // Since code was already implemented but failed review, rollback might be called
      // Note: The actual rollback behavior depends on the review failure handling in pipeline
    });
  });
  ...
  describe("Safety Violations", () => {
    it("should handle safety violations in issue validation", async () => {
      ...
      // Safety violations trigger rollback through general error handler
      expect(mockRollbackToCheckpoint).toHaveBeenCalledWith(
        "abc123def456",
        { cwd: "/tmp/project/.aq-worktrees/42-test", gitPath: "git" }
      );
  ...
  describe("External Service Failures", () => {
    it("should handle GitHub API failures", async () => {
      ...
      // GitHub API failures trigger rollback through general error handler
      expect(mockRollbackToCheckpoint).toHaveBeenCalledWith(
        "abc123def456",
        { cwd: "/tmp/project/.aq-worktrees/42-test", gitPath: "git" }
      );
  ...
  describe("State Transitions", () => {
    it("should capture failure state transitions", async () => {
      ...
      // Core loop failure should trigger rollback (checkpoint exists, rollbackStrategy is "all")
      expect(mockRollbackToCheckpoint).toHaveBeenCalledWith(
        "abc123def456",
        { cwd: "/tmp/project/.aq-worktrees/42-test", gitPath: "git" }
      );
