...
describe("createDraftPR", () => {
  ...
  it("should return null on failure", async () => {
    ...
    try {
      await createDraftPR(prConfig, ghConfig, ctx, { cwd: "/tmp", promptsDir: "/tmp" });
    } catch (error) {
  ...
  it("should skip in dry run mode", async () => {
    const result = await createDraftPR(prConfig, ghConfig, ctx, { cwd: "/tmp", promptsDir: "/tmp", dryRun: true });
