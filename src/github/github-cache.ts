 ...
 * | 데이터 종류 | TTL     | 키 형식                      |
 * |------------|---------|------------------------------|
 * | Issue 메타 | 5분      | `issue:{repo}:{number}`      |
 * | PR 메타    | 5분      | `pr:{repo}:{number}`         |
 *
 * ### TTL 근거
 * 파이프라인 평균 실행 시간(~3분) 내에서는 메타데이터 일관성을 보장하고,
 * 5분 후에는 외부에서 변경된 이슈/PR 메타데이터(본문, 라벨, 제목)를 반영합니다.
 * TTL 없이 무한 캐시하면 동일 프로세스 내 재처리 시 stale 데이터를 사용할 수 있습니다.
 *
 * ### 선택적 무효화
 * `deleteCached(key)`를 직접 호출하거나, issue-fetcher.ts의
 * `invalidateIssueCache` / `invalidatePRCache` 헬퍼를 사용합니다.
 * 무효화 시점: 파이프라인 재처리 전, 또는 메타데이터 변경이 확인된 경우.
 *
 * ### 수명
 * 캐시는 프로세스 메모리에 유지되며, `clearCache()`는 파이프라인 종료 시 호출합니다.
 */
