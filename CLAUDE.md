...
src/
  cli.ts              # CLI 진입점
  pipeline/           # 상태머신 기반 파이프라인 (orchestrator, core-loop, phase-executor)
...
- 워크플랜: `.claude/workplan/`
- quality-8 진행 중 (any 제거, 에러 표준화, API Zod, 보안 점검)
- quality-9 준비됨 (E2E, 커버리지 80%, 성능, 문서)
- multi-repo 준비됨 (리소스 격리, 설정 오버라이드, 모니터링)
