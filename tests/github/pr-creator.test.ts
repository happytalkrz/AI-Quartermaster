...
describe("createDraftPR", () => {
  ...
  it("should create PR with correct arguments", async () => {
    ...
    const result = await createDraftPR(prConfig, ghConfig, ctx, { cwd: "/tmp", promptsDir: "/tmp/prompts" });
  ...
  it("should throw on failure", async () => {
    ...
    await expect(createDraftPR(prConfig, ghConfig, ctx, { cwd: "/tmp", promptsDir: "/tmp/prompts" })).rejects.toThrow("Failed to create PR: error");
  });

  it("should skip in dry run mode", async () => {
    const dryConfig = { ...prConfig };
    const dryGh = { ...ghConfig };
    const result = await createDraftPR(dryConfig, dryGh, ctx, { cwd: "/tmp", promptsDir: "/tmp/prompts", dryRun: true });
