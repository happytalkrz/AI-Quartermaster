...
각 에이전트에게는 이슈 정보, 구현 계획, 변경된 코드를 제공하고, 다음 항목들을 검토하도록 지시하세요:

**Agent tool 사용 예시:**
```
Agent tool을 사용하여 병렬 리뷰를 수행합니다:

1. code-reviewer 에이전트: 전체적인 코드 품질과 요구사항 충족도를 검토
2. security-reviewer 에이전트: 보안 취약점과 안전성을 검토

각 에이전트의 결과를 종합하여 최종 verdict와 findings를 결정합니다.
```
